import React from 'react';
import { Product } from '../interfaces';

interface Props {
	product: Product;
}

function Item({ product }: Props) {
	const { product_id, product_name, category, price, price_type } = product;
	return (
		<div
			key={product_id}
			className="flex flex-col justify-start items-start px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full"
		>
			<div className="mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full">
				<div className="pb-4 md:pb-8 w-full md:w-40">
					<img
						src="https://i.ibb.co/84qQR4p/Rectangle-10.png"
						alt="dress"
						className="w-full hidden md:block"
					/>

					<img
						className="w-full md:hidden"
						src="https://i.ibb.co/L039qbN/Rectangle-10.png"
						alt="dress"
					/>
				</div>
				<div className="border-b border-gray-200 md:flex-row flex-col flex justify-between items-start w-full pb-8 space-y-4 md:space-y-0">
					<div className="w-full flex flex-col justify-start items-start space-y-8">
						<h3 className="text-xl  xl:text-2xl font-semibold leading-6">
							{product_name}
						</h3>
						<div className="flex justify-start items-start flex-col space-y-2">
							<p className="text-sm leading-none">
								<span>Category: </span> {category}
							</p>
							<p className="text-sm leading-none">
								<span>Size: </span> Small
							</p>
							<p className="text-sm leading-none">
								<span>Color: </span> Light Blue
							</p>
						</div>
					</div>
					<div className="flex justify-between space-x-8 items-start w-full">
						<p className="text-base xl:text-lg font-semibold leading-6">
							{`KES.${price} ${price_type}`}
						</p>
					</div>
				</div>
			</div>
		</div>
	);
}

export default Item;
